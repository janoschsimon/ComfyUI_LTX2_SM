...
from .gpu_model import gpu_model
from .helpers import (
    cleanup_memory,
    create_noised_state,
    generate_enhanced_prompt,
)
from .samplers import euler_denoising_loop
from .types import Denoiser, ModalitySpec
...
class DiffusionStage:
    ...
    def __init__(
        ...
        self.use_gguf=True if checkpoint_path.endswith(".gguf") else False
    ...
    def _build_transformer(self, *, device: torch.device | None = None, **kwargs: object) -> X0Model:
        ...
        return X0Model(builder.build(device=target, use_gguf=self.use_gguf,**kwargs)).to(target).eval()
...
class PromptEncoder:
    ...
    def __init__(
        ...
        self._dtype = dtype
        self._device = device
        self.cur_device = torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
    ...
    def _text_encoder_ctx(
        ...
        return gpu_model(self._text_encoder_builder.build(device=self._device, dtype=self._dtype,use_gguf=self.use_gguf).eval())
    ...
    def __call__(
        ...
        *,
        ...
        with gpu_model(
            self._embeddings_processor_builder.build(device=self._device, dtype=self._dtype,use_gguf=self.use_gguf).to(self._device).eval()
...
class ImageConditioner:
    ...
    def __init__(
        ...
        self._encoder_builder = Builder(
            ...
            registry=registry or DummyRegistry(),
            load_model="imageconditioner",
        )

    def _build_encoder(self) -> VideoEncoder:
        return self._encoder_builder.build(device=self._device, dtype=self._dtype,use_gguf=True).to(self._device).eval()
...
class VideoUpsampler:
    ...
    def __init__(
        ...
        if checkpoint_path is not None:
            self._encoder_builder = Builder(
                ...
                model_sd_ops=VAE_ENCODER_COMFY_KEYS_FILTER,
                registry=registry or DummyRegistry(),
                load_model="upsampler",
            )
        else:
            self._encoder_builder = None
    ...
    def __call__(self, latent: torch.Tensor) -> torch.Tensor:
        ...
        if self._encoder_builder is not  None:
            with (
                gpu_model(
                    self._encoder_builder.build(device=self._device, dtype=self._dtype,use_gguf=True).to(self._device).eval()
                ) as encoder,
                gpu_model(
                    self._upsampler_builder.build(device=self._device, dtype=self._dtype,use_gguf=True).to(self._device).eval()
                ) as upsampler,
            ):
            ...
            with (
                ...
                return upsample_video(latent=latent, video_encoder=self.video_encoder, upsampler=upsampler)
...
class VideoDecoder:
    ...
    def __call__(
        ...
        decoder = self._decoder_builder.build(device=self._device, dtype=self._dtype,use_gguf=True).to(self._device).eval()
...
class AudioDecoder:
    ...
    def __call__(self, latent: torch.Tensor) -> Audio:
        ...
        with (
            gpu_model(
                self._decoder_builder.build(device=self._device, dtype=self._dtype,use_gguf=True).to(self._device).eval()
            ) as decoder,
            gpu_model(
                self._vocoder_builder.build(device=self._device, dtype=self._dtype,use_gguf=True).to(self._device).eval()
            ) as vocoder,
        ):
